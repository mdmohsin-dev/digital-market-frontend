...
import { ORDER_STORAGE_KEY, type Order } from "@/lib/orders";

const ORDER_STATUS_UPDATED_EVENT = "ORDER_STATUS_UPDATED";

type TrackableOrder = Order & Partial<OrderTrackingData>;

const statusIcons: Record<TrackingStatus, typeof Check> = {
    accepted: Check,
    processing: Clock3,
    "on-the-way": Truck,
    "with-deliveryman": UserRound,
...
export default function TrackOrderPage() {
    ...
    const [foundOrder, setFoundOrder] =
        useState<TrackableOrder | null>(null);
    ...
    const refreshFoundOrder = () => {
        ...
        const updatedOrder = findOrderById(
            foundOrder.id,
        );
    ...
    const handleSearch = (
        ...
        const order = findOrderById(normalizedId);
    ...
    const trackingData = useMemo<OrderTrackingData | null>(
        () => {
            if (!foundOrder) {
                return null;
            }
            ...
            /*
             * Stored tracking data is supported for the future.
             * If the backend later returns trackingStatus/history,
             * the same page can consume them directly.
             *
             * For the current frontend-only stage, order.status
             * is the source of truth.
             */
            if (
                foundOrder.trackingStatus &&
    ...
    const currentEvent = trackingData
    ...
    return (
        <main className="min-h-screen bg-neutral-50">
            ...
            <section className="mx-auto max-w-350 px-4 py-10 sm:px-6 lg:px-8">
                ...
                {hasSearched && !foundOrder && (
                    <div className="mx-auto max-w-3xl rounded-2xl border border-red-200 bg-white px-6 py-12 text-center">
                        ...
                        <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-neutral-500">
                            We could not find an order with the
                            ID{" "}
                ...
                {foundOrder && trackingData && currentStep && (
                                            ...
                                            (
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div
                                                            className={`
                                                                flex h-10 w-10 items-center justify-center rounded-full border-2 bg-white transition-all duration-500
                                                                ${isCompleted
                                        ...
                                        (
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div
                                                        className={`
                                                            relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 bg-white transition-all duration-500
                                                            ${isCompleted
                                                                ? "border-primary bg-primary text-white"
                                                                : isCurrent
